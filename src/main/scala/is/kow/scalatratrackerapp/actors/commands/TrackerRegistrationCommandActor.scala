...
object TrackerRegistrationCommandActor {
  def props(commandPrefix: CommandPrefix, channelProjectActor:ActorRef) = Props(new TrackerRegistrationCommandActor(commandPrefix, channelProjectActor))
}
...
/**
  ...
  */
class TrackerRegistrationCommandActor(commandPrefix: CommandPrefix, channelProjectActor:ActorRef) extends Actor with ActorLogging {
        ...
        case fullRegex(registerProjectId) =>
          ...
          Option(registerProjectId) match {
            ...
            case None =>
              ...
              typing(smp.getChannel)
              context.actorOf(RegistrationActor.props(channelProjectActor)) ! RegistrationActor.ChannelQueryRequest(smp, ChannelProjectActor.ChannelQuery(smp.getChannel))
              context.become(awaitingRegistrationResponse)
          }
        case _ =>
